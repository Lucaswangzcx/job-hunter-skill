...
这是 `Job Hunter Skill` 的首个公开 `alpha` 版本。

它是一个基于 `DrissionPage + CDP 端口接管` 的自动化求职投递工具，当前支持：

- `Boss直聘`
- `实习僧`

这个版本的重点不是堆功能，而是先把真实可跑通的核心链路做稳：
...
- 这是 `alpha` 版本
- 暂不支持 `51job`
- 真实站点 selector 未来可能变化
- 仍需用户手动登录目标平台
- 建议先始终使用 `rehearsal` 再切换到 `apply`
