...
- 接管前必须确认对应 CDP 端口已监听；端口未打开时直接报错，不自动新开浏览器。
...
- 搜索词优先使用本次命令的 `--job`；未提供时使用 `config.json` 中 `target_roles` 的第一个岗位。
- 本次 `--job` 会临时加入评分用 `target_roles`，避免搜索词与岗位加分不一致。
- 默认端口：
  - Boss直聘：`9222`
  - 实习僧：`9223`
...
2. 让用户先把简历保存为 `resume.md`，放到运行目录。运行目录由 `--skill-dir`、`JOB_HUNTER_HOME` 或当前目录决定。

```powershell
Copy-Item examples/resume.example.md resume.md
```

3. 第一次运行时，如果没有 `config.json`，入口脚本会进入首次引导：
...
如果端口未监听，skill 会停止并提示用户重新启动浏览器，不会让 DrissionPage 自动打开新浏览器。
...
运行时如果未提供 `--platform`，先询问用户使用 Boss直聘 还是 实习僧。随后使用本次 `--job` 搜索岗位；未提供 `--job` 时使用 `config.json` 中 `target_roles` 的第一个岗位。执行前必须确认对应 CDP 端口已经由用户手动打开并登录。

## 评分配置

评分标准放在 `config.json` 的 `scoring` 段，用户可以按自身情况调整：

- `role_title_score`：岗位标题命中 `target_roles` 的加分。
- `skill_score_each` / `skill_score_cap`：每个 `skills` 命中的加分和总上限。
- `llm_score_min` / `llm_score_max`：LLM 或启发式补分范围。
- `heuristic_*`：未配置 LLM 时的补分规则。
